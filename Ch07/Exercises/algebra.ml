...
module RingOfPreRing (R : PreRing) : Ring with type t = R.t = struct
  include R
  ...
  let of_int n =
    ...
    let rec loop n b x =
      if n = 0 then x
      else loop Stdlib.(n / 2) (b * two) (if n mod 2 = 0 then x else x + b)
    in
    let m = loop (abs n) one zero in
    if n < 0 then -m else m
end

module FieldOfPreField (F : PreField) : Field = struct
